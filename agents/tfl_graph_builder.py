...
station_cache = {}
accessibility_features_of_interest = {
    "AccessViaLift",
        ...
        for line in lines:
            ...
            for s in station_data:
                ...
                features_of_interest = [
                    f["key"]
                    for f in features
                    if f["key"] in accessibility_features_of_interest
                    and f.get("value") == "Yes"
                ]
            ...
            for seq_data in routes_data:
                for sp_seq in seq_data.get("stopPointSequences", []):
                    ...
                    stop_points = (
                        sp_seq.get("stopPoint") or sp_seq.get("stopPoints") or []
                    )
                    if not stop_points:
                        continue
                    station_sequence = [
                        sp.get("id") for sp in stop_points if sp.get("id")
                    ]
                    ...
                    """,
                        {
                            "routeId": route_id,
                            "start": starting_station,
                            "end": terminating_station,
                        },
                    )
...
if __name__ == "__main__":
    create_graph()
    print(
        "London Underground graph with stations, accessibility features, and routes loaded into Neo4j."
    )
